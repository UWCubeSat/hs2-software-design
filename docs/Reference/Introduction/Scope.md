...
The detailed design for camera managers, star-tracker management, radiation tolerance, and storage/data-retention policy will be added when those designs are available. The Level 4 system-services and control layer contains the satellite state machine, deployment-wide F' services, and fatal handling; hardware-reset ownership remains with the application-manager interfaces described in that layer.
