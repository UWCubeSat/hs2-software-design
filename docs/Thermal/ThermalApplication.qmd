...
| HS2-THA-002 | While in `ACTIVE_HEATING`, ThermalApplication shall run a PID control loop each tick (error = `SetpointC` minus the average of all valid sensor readings; duty = `Kp`*error + `Ki`*accumulated error + `Kd`*(error - previous error)) and command the result, clamped to the 0-100 percent range, to `HeaterManager` | Unit test |
