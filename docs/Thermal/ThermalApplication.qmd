...
**Thermal software topology**:

![Thermal application topology diagram](../../Thermal/images/ThermalApplicationTopology.png){width=100%}
...
| HS2-THA-002 | While in `ACTIVE_HEATING`, ThermalApplication shall run a PID control loop each tick using the difference between the setpoint and the average of valid sensor readings, combine proportional, integral, and derivative terms, and command the result to HeaterManager after clamping it to the 0-100 percent range. | Unit test |
...
|---|---|---|
| `SET_HEATER_OVERRIDE` | dutyPercent (F32) | Clamps `dutyPercent` to the 0-100 percent range, engages the override (stored, applied by every subsequent `commandHeaterDuty` call regardless of state), logs `OverrideEngaged` with the clamped value, responds `OK` unconditionally. |
...
| `SENSOR_COLD_THRESH` (one per sensor) | `F32` | - | Per sensor minimum used to decide whether to activate heaters. |
| `SENSOR_HOT_THRESH` (one per sensor) | `F32` | - | Per sensor maximum used to emit a fatal warning to shut off nearby components. |
