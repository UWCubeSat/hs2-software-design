...
| `magnetorquerEnabled` | sync | input | `mtToggle_p` with is_locked boolean | Latches `magnetometerLocked` and forwards `Present` / `NotPresent` to the sensor's `knownMagneticDisturbance`. |
...
##### Configuration parameters {.unnumbered .unlisted}
