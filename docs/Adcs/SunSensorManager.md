...
| HS2-SSM-004 | SunSensorManager shall read all six sun sensor channels each rate-group tick while in RUN | Inspection |
...
| HS2-SSM-006 | SunSensorManager shall publish the six calibrated channel intensities to AdcsApplication after each successful read cycle | Inspection |
...
|-----------|------|-------------|
| `CALIBRATION_SCALE_0`…`CALIBRATION_SCALE_5` | `F32` (×6) | Per-channel sensitivity scale factors |
...
WAIT_RESET
  on tick: increment wait-tick counter
           if counter >= RESET_WAIT_TICKS:
             load CALIBRATION_SCALE_0-5, CALIBRATION_OFFSET_0-5 from PrmDb
             → RUN
           (no bus operations performed in this state)

RUN
  on tick: for each of the 6 channels:
           ...
           if all reads OK:
             ...
             publish sunIntensitiesOut (6 calibrated intensities, timestamp)
...
**Parameter reconfiguration:** When `parameterUpdated()` is called while `SunSensorManager` is in `RUN`, it reloads `CALIBRATION_SCALE_0-5` and `CALIBRATION_OFFSET_0-5` from `PrmDb` in place. While in `RESET` or `WAIT_RESET`, the signal is ignored, since parameters are freshly loaded on the next `WAIT_RESET`-to-`RUN` transition.
