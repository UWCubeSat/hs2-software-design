...
|---|---|---|---|---|
| `setLock` | sync | input | `mtToggle_p` with is_locked boolean | Requests the actuators be locked (`true`) or unlocked (`false`). |
...
| `setDutycycle` | — | output, 3-element array | `mtDutycycle_p` with dc F32 | Commanded duty-cycle magnitude for each actuator, clamped to its configured minimum and maximum. |
| `setDirection` | — | output, 3-element array | `mtDirection_p` with is_high boolean | Commanded current direction for each actuator (`true` for a non-negative computed duty cycle). |
