...
| `cameraPower` (2 instances) | Output | DataCollection.CameraPower for camera and powerOn, returning Types.CameraStatus | Power `lostCameraManager`/`foundCameraManager` (port index 0/1) on or off. |
| `cameraCheckup` (2 instances) | Output | DataCollection.CameraCheckup for camera, returning Types.CameraStatus | Per-camera health check, used by `HealthCheck` mode's `checkup` action. |
| `cameraCapture` (2 instances) | Output | DataCollection.CameraCapture for camera and imageType, returning Types.CameraStatus | Triggers one camera's capture of the current experiment's `imageType`.  |
| `positionGet` | Output | DataCollection.PositionGet returning Types.PositionData | Requests the current estimated position vector |
| `attitudeGet` | Output | DataCollection.AttitudeGet returning Types.Quaternion | Requests the current estimated attitude quaternion. |
| `attitudeRequest` | Output | DataCollection.AttitudeRequest returning Types.Quaternion | Requests an attitude quaternion. |
