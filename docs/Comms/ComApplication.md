...
| `configureGroupRate` | Output | <code>Svc.<br>ConfigureGroupRate</code> | Configure rate at which certain telemetry sections are transmitted as per the `Svc.TlmPacketizer` component |
***
...
| State | Entry action |
|-------|--------------|
| `BEACON` | Set all packets except SOH in the `TlmPacketizer` to `SILENCED`; set `downlinkMode` telemetry to `BEACON`. |
| `STORED_PLAYBACK` | Set SOH telemetry in the `TlmPacketizer` to `EVERY_MAX`; set `downlinkMode` telemetry to `STORED_PLAYBACK`. |
| `STANDARD_DOWNLINK` | Set all packets except SOH in the `TlmPacketizer` to `ON_CHANGE_MIN`; set `downlinkMode` telemetry to `STANDARD_DOWNLINK`. |
| `NO_DOWNLINK` | Set all packets in the `TlmPacketizer` to `SILENCED`; set `downlinkMode` telemetry to `NO_DOWNLINK`. |
...
The following diagram outlines the receiving data path.
